/* Staff pages: assigned bookings, service status, availability */
...
function renderDash(u, bs) {
  ...
  $("#completed").textContent = bs.filter(
    (b) => b.staffId === u.employeeId && b.status === "Completed",
  ).length;
  const avail = $("#availability");
  if (avail)
    avail.textContent = tAvail(
      (get("staff").find((x) => x.employeeId === u.employeeId) || u).status,
    );
}
...
function renderAssigned(u) {
  ...
  function draw() {
    ...
    state.page = pg.page;
    $("#assignedTable").innerHTML = pg.slice.length
      ? pg.slice
          .map(
            (b) =>
...
function staffMember(u) {
  ...
  if (!u) return null;
  return (
    list.find((x) => x.employeeId && x.employeeId === u.employeeId) || null
  );
}
...
function renderManagement(u) {
  ...
  function draw() {
    ...
    const pg = paginate(a, state.page);
    state.page = pg.page;
    $("#management").innerHTML = pg.slice.length
      ? pg.slice
          .map((b) => {
...
function renderAvailability(u) {
  const s = get("staff"),
    m = s.find((x) => x.employeeId === u.employeeId);
  if (!m) return;
  $("#astatus").textContent = tAvail(m.status);
  $("#toggle").textContent =
    String(m.status || "") === "Available"
      ? t("setUnavailable")
      : t("setAvailable");
  $("#toggle").onclick = () => {
    m.status =
      String(m.status || "") === "Available" ? "Unavailable" : "Available";
    set("staff", s);
    const sess = session() || {};
