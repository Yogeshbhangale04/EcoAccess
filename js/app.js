const $=s=>document.querySelector(s),$$=s=>[...document.querySelectorAll(s)];function get(k,d=[]){try{return JSON.parse(localStorage.getItem(k))??d}catch{return d}}function set(k,v){localStorage.setItem(k,JSON.stringify(v))}function id(p){return p+Date.now().toString().slice(-7)}function toast(m,error=false){let x=$('#toast');if(x){x.textContent=m;x.className=error?'toast error':'toast';setTimeout(()=>x.className='',3000)}}function session(){return get('session',null)}function logout(){localStorage.removeItem('session');location.href='../index.html'}function setActiveNav(){const current=location.pathname.split('/').pop()||'index.html';document.querySelectorAll('.app aside nav a').forEach(link=>{const target=link.getAttribute('href')?.split('#')[0].split('?')[0];link.classList.toggle('active',target===current)})}
...
document.addEventListener('DOMContentLoaded',()=>{$('[data-logout]')?.addEventListener('click',logout);$('#menu')?.addEventListener('click',()=>document.querySelector('aside')?.classList.toggle('open'));setActiveNav();if(session()&&$('#user'))$('#user').textContent=session().name||'User';if($('#loginForm'))initLogin();if($('#registerForm'))initRegister()});function initLogin(){
  let role='passenger';
  ...
  function validateIdentity(){
    ...
    if(role==='passenger'&&!/^\d{10}$/.test(value))return loginError('Mobile number must contain exactly 10 digits.');
    if(role==='admin'&&!/^\S+@\S+\.\S+$/.test(value))return loginError('Enter a valid email address.');
    return true;
  }
  function validatePassword(){
    if(!$('#password').value)return loginError('Password is required.');
    if(!passwordPattern.test($('#password').value))return loginError('Password needs 8+ characters, uppercase, lowercase and number.');
    return true;
  }
  ...
  $('#loginForm').onsubmit=e=>{
    e.preventDefault();
    if(!validateIdentity()||!validatePassword())return;
    ...
    if(!u){loginError(role==='staff'?'Staff login failed. Use Employee ID STF1001 and password Staff@123.':'Invalid credentials.');return}
    set('session',{...u,role});
