window.initBooking=function(){
  ...
  if($('#pnr')) $('#pnr').value=String($('#pnr').value||'').replace(/\D/g,'').slice(0,10);
  if($('#train')) $('#train').value=String($('#train').value||'').replace(/\D/g,'').slice(0,5);
  if($('#platform')) $('#platform').value=String($('#platform').value||'').replace(/\D/g,'').slice(0,2);
  show(1);
  function unitRate(){
  ...
  function usedCount(serviceName,station){
    return get('bookings').filter(x=>x.service===serviceName&&x.station===station&&x.status!=='Completed'&&x.status!=='Rejected')
  ...
  function isStaffFree(x){
    const s=String(x.status||'Available').trim().toLowerCase();
    return s!=='unavailable'&&s!=='busy'&&s!=='offline';
  }
  function availablePorters(){
    const porters=get('staff',[]).filter(x=>isPorterStaff(x)&&isStaffFree(x));
    const busy=get('bookings').filter(x=>x.service==='Porter'&&x.status!=='Completed'&&x.status!=='Rejected'&&x.staffId)
      .map(x=>x.staffId);
    return Math.max(0,porters.filter(x=>!busy.includes(x.employeeId)).length);
  }
  function applyPassengerLimit(){
    const input=$('#passengerCount'),label=input?.closest('label');
  ...
  function showAvailability(){
    ...
    const ok=count>0;
    $('#avail').innerHTML=`${ok?'✓':'✗'} ${service} ${ok?'is':'is not'} available at ${station}.<br>${line}`;
    applyPassengerLimit();
    const pick=$('#pickPlatform');
    if(pick&&!pick.value) pick.value=$('#platform').value||'';
  }
  $('#next1').onclick=()=>{
    const pnrErr=demoPnrError($('#pnr').value);if(pnrErr)return toast(pnrErr,true);
    const trainErr=trainNumberError($('#train').value);if(trainErr)return toast(trainErr,true);
    const platErr=platformError($('#platform').value,'Platform number');if(platErr)return toast(platErr,true);
  ...
  $('#next3').onclick=()=>{
    const pick=$('#pickPlatform').value,drop=$('#dropPlatform').value,station=$('#station').value;
    const pickErr=platformError(pick,'Pick platform');if(pickErr)return toast(pickErr,true);
    const dropErr=platformError(drop,'Drop platform');if(dropErr)return toast(dropErr,true);
    if(pick===drop)return toast('Pick and drop platforms must be different.');
    if(service==='Porter'){
      if(availablePorters()<1)return toast('No porters are available.',true);
    }else{
      const count=Number($('#passengerCount').value);
      if(!Number.isInteger(count)||count<1)return toast('Enter a valid number of passengers.');
      if(service==='Inter Vehicle'){
        if(availableVehicles(station)<1)return toast('No vehicles are available at this station.',true);
        if(count>8)return toast('Enter between 1 and 8 passengers.');
      }
      if(service==='Wheelchair'){
        const avail=availableWheelchairs(station);
        if(avail<1)return toast('No wheelchairs are available at this station.',true);
        if(count>avail)return toast(`Only ${avail} wheelchair(s) available. Reduce the number of passengers.`,true);
      }
    }
    base=calcBase();
  ...
  function selectedPay(){return $$('input[name="pay"]').find(i=>i.checked)?.value||'upi'}
  ...
  $('#pay').onclick=()=>{
    ...
    const member=typeof assignableStaff==='function'?assignableStaff(service):null;
    let b={id:id('BK-'),passengerId:u.id,passenger:u.name,service,station:$('#station').value,platform:platVal,date:$('#date').value,time:'10:30',fare:pay,grossFare:g,discount:disc,couponCode:c?c.code:'',status:member?'Assigned':'Booked',staffId:member?member.employeeId:'',train:$('#train').value,bags,weightRange,passengerCount:service==='Porter'?1:currentCount(),pickPlatform:pickVal,dropPlatform:dropVal};
    ...
    $$('.steps [data-step]').forEach(el=>el.classList.remove('active','done'));
