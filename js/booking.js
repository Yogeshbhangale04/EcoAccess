document.addEventListener('DOMContentLoaded',()=>{let u=requireJourney();if(!u)return;if($('#next1')&&typeof initBooking==='function')initBooking();if($('#track'))initTracking()});
function show(n){
...
function initTracking(){
  ...
  function showBooking(b){
    ...
    if(b.status==='Rejected'){
      $('#timeline').innerHTML=`<p class="done"><b>✓ Booked</b></p><p class="done reject-step"><b>✕ Rejected</b></p>`;
    }else{
      const ci=order.indexOf(b.status);
      $('#timeline').innerHTML=order.map((s,i)=>`<p class="${i<=ci?'done':''}"><b>${i<=ci?'✓':'○'} ${s}</b></p>`).join('');
    }
    $('#details').innerHTML=`<p><b>Booking:</b> ${b.id}</p><p><b>Passenger:</b> ${b.passenger}</p><p><b>Number of Passenger:</b> ${b.passengerCount||1}</p><p><b>Service:</b> ${b.service}</p><p><b>Train:</b> ${b.train}</p><p><b>Station:</b> ${b.station}</p><p><b>Pick & Drop:</b> ${bookingRoute(b)}</p><p><b>Status:</b> ${b.status}</p><p><b>Fare:</b> ₹${b.fare}${b.discount?` (₹${b.discount} off with ${b.couponCode})`:''}</p>`;
  }
  function draw(){
