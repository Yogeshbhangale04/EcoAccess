...
function initTracking() {
  ...
  function showBooking(b) {
    ...
    $("#details").innerHTML =
      `<p><b>Booking:</b> ${b.id}</p><p><b>Passenger:</b> ${b.passenger}</p><p><b>Number of Passenger:</b> ${b.passengerCount || 1}</p><p><b>Service:</b> ${b.service}</p><p><b>Train:</b> ${b.train}</p><p><b>Station:</b> ${b.station}</p><p><b>Pick & Drop:</b> ${bookingRoute(b)}</p><p><b>Status:</b> ${b.status}</p><p><b>Fare:</b> ₹${b.fare}${b.discount ? ` (₹${b.discount} off with ${b.couponCode})` : ""}</p>`;
  }
  function draw() {
    const u = session(),
      q = listQuery("#search");
    const a = get("bookings").filter(
      (x) =>
        x.passengerId === u.id &&
        textMatch(q, x.id, x.service, x.status, x.station),
    );
    ...
    $("#bookingList").innerHTML = pg.slice.length
      ? pg.slice
          .map(
            (b) =>
              `<div class="listrow"><div><b>${b.id}</b> · ${b.service} · ${b.date}<br><span class="badge">${b.status}</span></div><button type="button" class="btn sm" data-track="${b.id}">View</button></div>`,
          )
          .join("")
